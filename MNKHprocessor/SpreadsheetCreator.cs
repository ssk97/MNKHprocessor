    ...
    class Spreadsheets {
        ...
        public static void RenderSpreadsheet(TurnData turn_data) {
            ...
            foreach (SectionData section in turn_data.sections) {
                ...
                if (Int32.TryParse(section.dice, out _)) {
                    sheet.Cells[top_row, 2].Formula = string.Concat("=MAX(0,D", top_row, "-", section.dice, ")");
                ...
                copyable_sheet.Cells[top_row, 1].Formula = string.Concat(
                    "=\"-[]", section.name, " (\"&Main!D", top_row, "&\"/", section.dice, " Dice, \"& Main!F", top_row, "&\" R)\"");
